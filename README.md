...
- 用户注册、登录和退出。
- 保存体重与每日目标热量。
- 按早餐、午餐、晚餐、加餐记录每次识别结果。
- 按日期查看每日总热量、剩余热量和餐别拆分。
...
当前版本使用服务端 JSON 文件保存账号、设置和餐食记录，文件路径默认为 `data/app-db.json`。密码会用 Node.js `scrypt` 哈希后保存，不会明文保存。

这个方案适合个人使用和功能验证。Render 免费服务的文件系统不适合作为长期数据库；后续如果要正式长期使用、多人使用或避免重新部署丢数据，建议升级到 Supabase、PostgreSQL 或其他托管数据库。
